...
class Phase6PendingActionVisibilityTests(unittest.TestCase):
    def setUp(self):
        fd, self.db_path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        oms.DATABASE = self.db_path
    ...
    def test_dashboard_stat_counts_pending_actions_across_cases(self):
        client = oms.app.test_client()
        self.login_admin(client)

        case_a = self.create_case()
        case_b = self.create_case()
        self.approve_latest_reasoning(client, case_a)  # queues one pending action
        self.approve_latest_reasoning(client, case_b)  # queues another

        response = client.get("/admin/finance")
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"Pending Actions", response.data)

        conn = self.connect()
        count = conn.execute("SELECT COUNT(*) AS c FROM case_action WHERE status = 'pending'").fetchone()["c"]
        conn.close()
        self.assertEqual(count, 2)

    def test_dispatching_action_removes_it_from_pending_count(self):
        client = oms.app.test_client()
        self.login_admin(client)
        case_id = self.create_case()
        ...
        before = client.get("/admin/finance")
        self.assertIn(b"1", before.data)  # sanity: at least one pending exists somewhere on the page
    ...
    def test_recent_actions_table_shows_status_pill(self):
        client = oms.app.test_client()
        self.login_admin(client)
        case_id = self.create_case()
        self.approve_latest_reasoning(client, case_id)
    ...
    def test_viewing_dashboard_does_not_mutate_pending_action_or_case(self):
        client = oms.app.test_client()
        self.login_admin(client)
        case_id = self.create_case()
        self.approve_latest_reasoning(client, case_id)
        ...
        conn = self.connect()
        action_before = conn.execute(
            "SELECT * FROM case_action WHERE case_id = ? AND action_type = 'controlled_action_queued' "
            "ORDER BY id DESC LIMIT 1",
            (case_id,),
        ).fetchone()
        case_before = conn.execute("SELECT * FROM financial_case WHERE id = ?", (case_id,)).fetchone()
        conn.close()

        client.get("/admin/finance")
        client.get(f"/admin/finance/case/{case_id}")

        conn = self.connect()
        action_after = conn.execute("SELECT * FROM case_action WHERE id = ?", (action_before["id"],)).fetchone()
        case_after = conn.execute("SELECT * FROM financial_case WHERE id = ?", (case_id,)).fetchone()
        conn.close()

        self.assertEqual(action_before["status"], action_after["status"])
        self.assertEqual(case_before["status"], case_after["status"])
        self.assertEqual(case_before["risk_tier"], case_after["risk_tier"])

    def test_no_pending_actions_shows_zero(self):
        client = oms.app.test_client()
        self.login_admin(client)
        self.create_case()  # analysis stages reasoning, but nothing is approved yet
